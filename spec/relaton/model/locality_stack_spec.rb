describe Relaton::Model::LocalityStack do
  it "from XML" do
    xml = <<~XML
    ...
    expect(locality_stack.locality.first.reference_from.content).to eq "1"
    expect(locality_stack.locality.last.type).to eq "page"
    expect(locality_stack.locality.last.reference_from.content).to eq "4"

describe Relaton::Bib::FullNameType do
  let(:fake_class) do
    Class.new(Lutaml::Model::Serializable) do
      include Relaton::Bib::FullNameType
  ...
  context "XML mapping" do
    it "with completename" do
      ...
      expect(item.abbreviation).to be_instance_of Relaton::Bib::FullNameType::Abbreviation
      expect(item.completename).to be_instance_of Relaton::Bib::FullNameType::Completename
    ...
    it "with name parts" do
      ...
      expect(item.prefix.first).to be_instance_of Relaton::Bib::FullNameType::Prefix
      expect(item.forename).to be_instance_of Array
      expect(item.forename.size).to eq 1
      expect(item.forename.first).to be_instance_of Relaton::Bib::FullNameType::Forename
      expect(item.initials).to be_instance_of Relaton::Bib::FullNameType::FormattedInitials
      expect(item.surname).to be_instance_of Relaton::Bib::FullNameType::Surname
    ...
    it "with variants" do
      ...
      expect(item.variant.first).to be_instance_of Relaton::Bib::FullNameType::Variant
      expect(item.to_xml).to be_equivalent_to xml
    end
  end
end

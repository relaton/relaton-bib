module Relaton
  module Model
    module LocalizedString
      ...
      class Content
        # @param elements [Array<String>, Variants]
        def initialize(elements = [])
          @elements = elements
        end
        ...
        def self.of_xml(node)
          ...
          elms = node.children.map { |n| n.to_xml }
          new elms
        end

        def add_to_xml(parent)
          @elements.each { |e| e.is_a?(String) ? parent << e : e.add_to_xml(parent) }
        end

        def to_s
          @elements.map { |e| e.is_a?(String) ? e : e.to_xml }.join
        end
      end

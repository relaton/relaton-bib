describe Relaton::Bib::Bibitem do
  let(:xml) do
    <<~XML
      <bibitem id="ID" type="standard" schema-version="v1.4.1">
        <fetched>2024-02-01</fetched>
  ...
  context "parse XML" do
    ...
    it "title" do
      expect(doc.title).to be_instance_of Array # Relaton::Bib::TitleCollection
      expect(doc.title.size).to eq 2
      expect(doc.title.first).to be_instance_of Relaton::Bib::Title
      expect(doc.title[0].content).to eq "<em>Main</em> Title"
      expect(doc.title[0].type).to eq "main"
      expect(doc.title[0].language).to eq "en"
      expect(doc.title[1].content).to eq "Subtitle"
    ...
    it "source" do
      ...
      expect(doc.source.first).to be_instance_of Relaton::Bib::Source
      expect(doc.source.first.content).to be_instance_of String # Addressable::URI
      expect(doc.source.first.content).to eq "http://example.com"
    ...
    it "docidentifier" do
      ...
      expect(doc.docidentifier.first.content).to eq "ISO 123"
    ...
    it "date" do
      ...
      expect(doc.date.first).to be_instance_of Relaton::Bib::Date
  ...
  xit "to XML" do
    ...
    item.title << title
    item.source << Relaton::Bib::Source.new(type: "src", content: "http://example.com")
    item.docidentifier << Relaton::Bib::Docidentifier.new(content: "ISO 123", type: "ISO")
    item.date << Relaton::Bib::Date.new(type: "published", on: "2020-01-01")
    item.date << Relaton::Bib::Date.new(type: "accessed", from: "2024-02-01", to: "2024-12-31")
